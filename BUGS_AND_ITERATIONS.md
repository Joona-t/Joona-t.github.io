...
## 2026-07-08: Fleet P2 audit — dead weight + Dyson swarm perf gate + favicon (P2-8/P2-4)

**Problem:** Flagship-site audit (`meta_implementation.md` Appendix A) flagged: (1) `initDysonSwarm()` unconditionally built ~1074 span elements on every single page load regardless of theme, only hidden post-hoc via CSS for retro/basalt — pure wasted construction cost on 2 of 4 themes; (2) `images/adhd.png` (4.1MB) shipped in the repo with zero references anywhere — pure dead weight; (3) the flagship domain had no favicon at all; (4) `<canvas id="spiral-canvas">` sat in the DOM with a CSS hide rule but zero drawing code anywhere in `script.js` — dead markup from a removed feature; (5) `.DS_Store` wasn't gitignored.
**Root cause:** (1) the swarm-building IIFE ran eagerly on load with no theme check — CSS-only hiding (`html.theme-retro .dyson-swarm{display:none}`, `html.theme-basalt .dyson-swarm{display:none!important}`) still paid the full DOM-construction cost for hidden themes. (2)/(4) leftover assets/markup from earlier design iterations never cleaned up after the features they backed were replaced. (3) never added during initial site build.
**Fix:** Refactored `initDysonSwarm()` → `dysonSwarmController()` in `script.js`: extracted the build into `buildDysonSwarm()`, gated behind `themeHidesSwarm()` (checks for `theme-retro`/`theme-basalt` on `<html>`), and deferred the whole thing via `requestIdleCallback` (200ms `setTimeout` fallback) so it never competes with first paint even on themes that do show it. Hooked the theme-pill click listeners to re-attempt the deferred build so switching from retro/basalt into candy/kawaii client-side still gets the swarm (build is idempotent — guards on `host.querySelector('.dyson-swarm')`). Verified via preview server across all 4 themes: candy/kawaii build 1074 dots, retro/basalt build 0, client-side switch retro→candy correctly builds on demand, no duplicate injection on repeat clicks. Deleted `images/adhd.png` (confirmed zero references via repo-wide grep). Generated a Sparky-mascot favicon from `images/sparky.png` (squared crop via macOS `sips`, no PIL available) at 16px/32px/180px (apple-touch-icon), added the three `<link rel="icon"/apple-touch-icon">` tags to `index.html:<head>`. Removed the dead `<canvas id="spiral-canvas">` element from `index.html`, its `display:none` rule from `styles.css`, and its now-pointless `html.theme-retro #spiral-canvas` positioning rule from `styles-retro.css`. Added `.DS_Store` to `.gitignore`. Note: the "compress remaining multi-MB heroes to WebP" quick win from the audit turned out to be moot for this repo — `adhd.png` was the only image over 1MB; nothing else qualified after its deletion (verified via `find . -size +1M`).
**Verification:** Local preview server (`joona-t-portfolio` launch config) — screenshot + `preview_eval` DOM checks across candy/kawaii/basalt/retro confirm swarm dot counts (1074 vs 0) match theme visibility, zero console errors/warnings in any state, all 3 favicon files + apple-touch-icon resolve 200 via fetch, `#spiral-canvas` absent from DOM and absent from all 3 stylesheets/HTML/JS (grep confirmed zero remaining references).
**Files:** script.js, index.html, styles.css, styles-retro.css, .gitignore, images/adhd.png (deleted), images/favicon-16.png (new), images/favicon-32.png (new), images/apple-touch-icon.png (new)
